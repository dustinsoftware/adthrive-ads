...
class Main {
	...
	public function add_options( $cmb ) {
		$cmb->add_field( array(
			...
			'desc' => 'Copies ads.txt to the site root daily. Only use if instructed to do so by AdThrive support.',
			'id' => 'copy_ads_txt',
			'type' => 'checkbox',
		) );
	...
	/**
	 * Deactivation hook - delete the ads.txt file
	 */
	...
	private function delete() {
		if ( \WP_Filesystem() ) {
			...
			if ( $wp_filesystem->is_file( $filename ) ) {
				...
				if ( strpos( $contents, 'AdThrive' ) !== false ) {
